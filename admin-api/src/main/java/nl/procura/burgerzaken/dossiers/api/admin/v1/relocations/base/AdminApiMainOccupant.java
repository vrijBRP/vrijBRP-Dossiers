...
public class AdminApiMainOccupant extends AdminApiPerson {

  @Override
  public Person toPerson(Dossier dossier) {
    Person person = super.toPerson(dossier);
    person.addRole(PersonType.MAIN_OCCUPANT);
    return person;
  }

  public static AdminApiMainOccupant of(Person person) {
    if (!person.containsRole(PersonType.MAIN_OCCUPANT)) {
      throw new IllegalArgumentException("Person doesn't have main occupant role");
    }
    return new AdminApiMainOccupant(
        AdminApiMainOccupant.builder()
