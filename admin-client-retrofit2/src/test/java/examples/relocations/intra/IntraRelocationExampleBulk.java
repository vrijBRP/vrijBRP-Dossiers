...
@Slf4j
public class IntraRelocationExampleBulk {
  ...
  private static void add(int increment, AdminApiIntraMunicipalRelocationsApi api)
      throws java.io.IOException {
    IntraMunicipalRelocation newRelocation = IntraMunicipalRelocationTest.createNew();
    assert newRelocation.getDossier() != null;
    newRelocation.getDossier().setStartDate(newRelocation.getDossier().getStartDate().minusDays(increment));
    newRelocation.getDossier().setEntryDateTime(LocalDateTime.now().plusMinutes(increment).withNano(0));
